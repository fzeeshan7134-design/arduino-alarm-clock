...
const unsigned long QUICK_TEST_MS = 5000UL;
...
void handleButtons() {
  ...
  if (currentMode == SET_HOURS) {
    ...
    if (digitalRead(BTN_UP) == LOW) {
      ...
      alarmTargetMillis = millis() + 
        (unsigned long)setHours * 3600000UL +
...
void updateDisplay() {
  ...
  if (currentMode == NORMAL) {
    ...
    if (alarmActive) {
      ...
      unsigned long remaining = (alarmTargetMillis > millis()) ? 
                                (alarmTargetMillis - millis()) : 0;
